---
title: "Hospital Workers Report"
author: "CSH"
date: "August 08, 2019 to September 08, 2019"
output:
  html_document:
    df_print: paged
    theme: spacelab
...
# 1. Update the metadata: change "date" to the date/date range for which you want to generate a # # # #    report. 
# Also, update the title of the markdown appropriately given who you are genereating this report for (IDI, Datamation, etc).
...
# 4. Update the block named "dateInput" to be the date/date range for which you would like to generate #    a report. Update the variables, 'dateInput', 'date_ll', 'date_ul'.
...
# 12. Check for updates in the scto variable named 'member_name' until sign up form is cleaned up. Given there are updates, change the teamMembers variable accordingly, and then run the code. Also, update the "helper.R" file's, 'getPartnerTeam' function after going through the member names and seeing if some new names are added.
...
# 15. After this, source the "generalReportScript.Rmd" file.
```
...
# team name can be one of: "IDI", "Datamation", "HospitalWorkers", "unknown", "d2dinhouse"
teamName <- "HospitalWorkers" 
teamMembers <- getPartnerTeam(teamName)
```


```{r dateInput, warning = FALSE}
dateInput <- "2019-08-08"

# date range vector for which the report is to be generated
date_ll <- "2019-08-05"
date_ul <- "2019-08-31"
tmp_getDateRange <- lubridate::date(date_ll) : lubridate::date(date_ul)
...
# File names for Google Sheets used in the vaccination card back checks and audio audits # # back checks sheet for each organization. If there are changes in  file names, change the file names in this block and then move ahead.
...
#
# 2. d2d in house:
#    2.1 Aud Audits: "v1 Audio Audits d2d in house team - v3 audio audits.csv"
#    2.2 Vacc Cards: "v1 Vaccination Card Photo Review d2d in house team - Vaccination Card.csv"

# 3. hospital workers:
#    3.1 Aud Audits: "v1 Audio Audits Hospital Workers - v3 audio audits.csv"
#    3.2 Vacc Cards: "v1 Vaccination Card Photo Review Hospital Workers - Vaccination Card.csv"

# 4. Datamation
#    4.1 Aud Audits: "v1 Audio Audits datamation - v3 audio audits.csv"
#    4.2 Vacc Cards: "v1 Vaccination Card Photo Review datamation - Vaccination Card.csv"



# save the file names corresponding to the value of the variable "teamName", that is set in the teamDecide block
audAudBackCheckFileName <- "v1 Audio Audits d2d in house team - v3 audio audits.csv"
vaccCardBackCheckFileName <- "v1 Vaccination Card Photo Review d2d in house team - Vaccination Card.csv"
...
#### Total Number of surveys done by each surveyor from `r date_ll` to `r date_ul`

&nbsp;

This graph displays the number of surveys done by each surveyor from `r date_ll` to `r date_ul`

```{r numSurveysPerSurveyorOverAPeriodOfTime, warning =  FALSE}

# getting data for the date range set above
db_ds_wide_expand_date_range <- filter(ds_wide_expand, dateOnly %in% dateRange)
...
db_ds_wide_expand_date_range <- arrange(db_ds_wide_expand_date_range, member_name)

# further filtering data to only include teamMembers correponding to the current teamName
db_ds_wide_expand_date_range <- filter(db_ds_wide_expand_date_range, member_name %in% teamMembers)

# setting x axis breaks
breaks_daily <- c(0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50)
breaks_weekly <- c(0, 20, 40, 60, 80, 100, 120, 140, 160, 180, 200, 220, 240, 260)
breaks_monthly <- c(0, 20, 40, 60, 80, 100, 120, 140, 160, 180, 200, 220, 240, 260)

breaks_longer <- c(0, 40, 80, 120, 160, 200, 240, 280, 320, 360, 400, 440, 480, 520)

# creating the plot for the number of surveys completed in the date range set above, by the team members belonging to "teamName"
db_daily_t1_g1 <- ggplot(db_ds_wide_expand_date_range, mapping = aes(x = member_name)) + geom_histogram(stat = "count") + coord_flip() + ggtitle(str_c("Number of Surveys done from:", as.character(date_ll), "to", as.character(date_ul), sep = " ")) + scale_y_continuous(breaks = breaks_longer) + geom_text(aes(label=..count..), stat="count", vjust= 0.5, hjust = 2, size= 5)
...
#### Average number of surveys per surveyor per day (date range: `r date_ll` to `r date_ul`)

&nbsp

This graph displays the average number of surveys done by each surveyor in a day.

```{r avgNumOfSurveysPerDay}

# after inputting the number of days in the "days_time_period" variable, this block of code calculates the average number of surveys done by each surveyor. This assumes total number of days worked for everyone is the same across all surveyors.
db_ds_wide_expand_date_range_grp <- group_by(db_ds_wide_expand_date_range, member_name)
days_time_period <- 7
db_ds_wide_expand_date_range_summary <- dplyr::summarise(db_ds_wide_expand_date_range_grp, avgNumSurv = round(n()/days_time_period))

ggplot(db_ds_wide_expand_date_range_summary, mapping = aes(x = member_name, y = avgNumSurv)) + geom_bar(stat = "identity") + coord_flip() + geom_text(mapping = aes(label = avgNumSurv), vjust= 0.5, hjust = 2, size= 5)
                                                         
```



#### Average duration of surveys per surveyor from `r date_ll` to `r date_ul`

&nbsp;

This graph displays the average time taken (in minutes) by the surveyor to complete a survey, in the surveys done from `r date_ll` to `r date_ul`

```{r avgTimePerSurBySurveyorPerDay}

# In the given date range what on average how long does it take for a surveyor to administer a survey.
db_ds_wide_expand_date_range_grp <- group_by(db_ds_wide_expand_date_range, member_name)
db_ds_wide_expand_date_range_summ <- dplyr::summarise(db_ds_wide_expand_date_range_grp, durOfSurvPerSurveyor = mean(durationOfSurveyInMinutes, na.rm = TRUE))

db_ds_wide_expand_date_range_summ <- filter(db_ds_wide_expand_date_range_summ, member_name %in% teamMembers)

    db_daily_t1_g4 <- ggplot(db_ds_wide_expand_date_range_summ, mapping = aes(x = member_name, y = durOfSurvPerSurveyor)) + geom_bar(stat = "identity") + coord_flip() + scale_y_continuous(breaks = seq(1, 50, by = 2.5)) + ggtitle(str_c("Average Survey duration in the following time period ", as.character(date_ll), "to", as.character(date_ul), sep = " ")) + geom_text(aes(label = round(durOfSurvPerSurveyor)), vjust= 0.5, hjust = 2, size= 5) + ylab("duration (in minutes)")
db_daily_t1_g4
...
# this is needed for counting total number of surveys that are less than a given "threshold" minutes.
threshold <- 10
```

&nbsp;

#### For each surveyor, what percent of surveys were less than `r threshold` minutes in the following date range `r date_ll` to `r date_ul`. In the table, replace 'x' with `r threshold`.
...
# Option 2: getting audio files for a Date Range
# 'dateRange': this is the variable that we have for setting date ranges. This can be found in the 'dateInput' block.
...
# furhter filtering out those rows in which duration of survey is less than trhe set threshold--
...
audioAuditDataClean <- filter(audioAuditDataClean, `Submission Date` %in% dateRange)
...
# create 2 dataframes, one for audio audit data and one for the enrollment survey data. After that compare both.
...
# Checking % mismatches in father's contact number in both datasets------
...
# # foo_tmp[43] <- "Section did not come up"
# # foo_tmp[161] <- "Section did not come up"
...
# updating colnames so that they are more intuitive
...
####  4. In the following date range (`r date_ll` to `r date_ul`): **for what number of surveys do we find a blank audio recording**
...
enrSurDataSub_tmp <- dplyr::filter(enrSurDataSub, dateOnly %in% dateRange)
enrSurDataSub <- dplyr::filter(enrSurDataSub, KEY %in% enrSurDataSub_tmp$KEY) # only for the given date range
...
# changing dates to relevant format
vaccCardPhotoReviewData$`Child 1 DOB` <- mdy(vaccCardPhotoReviewData$`Child 1 DOB`)
vaccCardPhotoReviewData$`Child DOB
Enter if:  "Is there a child DOB" == 1` <- ymd(vaccCardPhotoReviewData$`Child DOB
Enter if:  "Is there a child DOB" == 1`)

# compare enrollment survey child DOB with vaccination card back check child DOB
vaccCardPhotoReviewData$eqChDOB <- vaccCardPhotoReviewData$`Child 1 DOB` == vaccCardPhotoReviewData$`Child DOB
Enter if:  "Is there a child DOB" == 1`

# summarising the data compared  in the above comment
vaccCardPhotoReviewData_grpSurv <- group_by(vaccCardPhotoReviewData, `Surveyor Name`)
vaccCardPhotoReviewData_summarise <- dplyr::summarise(vaccCardPhotoReviewData_grpSurv, countMismatch = count_if("FALSE", eqChDOB), totalNonNA = (n() - sum(is.na(eqChDOB))), totalRowsIncNA = n())
vaccCardPhotoReviewData_summarise <- mutate(vaccCardPhotoReviewData_summarise, 
                                            percentMismatch = countMismatch/totalNonNA)
write.csv(vaccCardPhotoReviewData_summarise, "chDOBComparisonSummary.csv")
...
photoQualDataSub_grp_survName_summary <- dplyr::summarise(photoQualDataSub_grp_survName, countBlur = count_if("Blur", PhotoQuality), countClearlyVisible = count_if("Clearly Visible", PhotoQuality), countNotAVaccCardPhoto = count_if("Not a picture of vaccination card",  PhotoQuality), n = n())
...
# looking at vaccinations only: equating the 2 datasets together and given that there is a date in both the datasets, finding out what %
...
# Only display the table if there is data in the table
...
dup_ds <- subset(dup_ds, dateOnly %in% dateRange)
...
ggplot2::ggplot(data = gps_col_summarise, mapping = aes(x = SurveyorName, y = `%MissGPSCoord`)) + geom_bar(stat = "identity") + geom_text(aes(label=`%MissGPSCoord`), vjust= 0.5, hjust = 2, size= 5) + coord_flip()
```
...
ggplot2::ggplot(data = dup_phn_num_by_surv_summarise_fath_nonNA_grp_mem_name_summ2, mapping = aes(x = SurveyorName, y = `%DupPhnNum`)) + geom_bar(stat = "identity") + geom_text(aes(label=`%DupPhnNum`), vjust= 0.5, hjust = 2, size= 5) + coord_flip()
